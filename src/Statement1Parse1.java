...
public final class Statement1Parse1 extends Statement1 {
    ...
    private static void parseIf(Queue<String> tokens, Statement s) {
        ...
        Condition con = parseCondition(tokens.dequeue());

        Reporter.assertElseFatalError(tokens.front().equals("THEN"),
                "Missing THEN");
        tokens.dequeue(); // "THEN"

        Statement child1 = s.newInstance();
        child1.parseBlock(tokens);

        if (tokens.front().equals("ELSE")) {
            tokens.dequeue(); // "ELSE"

            Statement child2 = s.newInstance();
            child2.parseBlock(tokens);
            ...
            s.assembleIfElse(con, child1, child2);
        } else {
            Reporter.assertElseFatalError(tokens.front().equals("END"),
                    "Invalid syntax");
            tokens.dequeue(); // "END"
    ...
    public void parse(Queue<String> tokens) {
        ...
        if (tokens.front().equals("IF")) {
            ...
            Reporter.assertElseFatalError(
                    Tokenizer.isIdentifier(tokens.front()), "Invalid");
